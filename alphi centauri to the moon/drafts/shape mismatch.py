...
def compute_mtf_contrast_ratio(mtf):
    ...
    cycs_per_pixel = np.fft.fftfreq(mtf.shape[0], d=1.0)[:mtf.shape[0]//2]  # Frequencies in cycles per pixel
    
    # Compute the contrast ratio by taking the magnitude of the MTF for each frequency
    contrast_ratio = np.sum(mtf[:mtf.shape[0]//2], axis=0)  # Sum the MTF magnitudes across the rows
