...
def compute_mtf_contrast_ratio(mtf):
    ...
    # Ensure contrast ratio is highest at 0 cycles per pixel
    contrast_ratio = contrast_ratio / np.max(contrast_ratio)  # Normalize to 1.0 at 0 cycles per pixel
    
    # Invert the contrast ratio for the plot to show decreasing contrast
    contrast_ratio = np.flip(contrast_ratio)
    return cycs_per_pixel, contrast_ratio

def adjust_clahe_with_mtf(image, mtf, contrast_ratio, base_clip_limit=2.0, tile_grid_size=(8, 8)):
    """
    Adjusts the CLAHE clipLimit based on the MTF contrast ratio.
    If the contrast ratio is high (indicating high frequency detail), increase the clipLimit.
    """
    # Compute an adjusted clipLimit based on the highest contrast ratio at high frequencies
    high_freq_contrast = np.max(contrast_ratio)  # Max contrast at higher frequencies
    adjusted_clip_limit = base_clip_limit * high_freq_contrast  # Scale clipLimit by contrast ratio
    
    # Apply CLAHE with the adjusted clipLimit
    clahe = cv2.createCLAHE(clipLimit=adjusted_clip_limit, tileGridSize=tile_grid_size)
    return clahe.apply(image)
...
def main(image1_path, image2_path):
    ...
    # Adjust CLAHE based on MTF contrast ratio
    enhanced_contrast_image = adjust_clahe_with_mtf(sharpened_image, mtf, contrast_ratio)
